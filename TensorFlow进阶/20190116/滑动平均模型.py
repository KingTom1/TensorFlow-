'''TensorFlow中提供ExponentialMovingAverage实现指数滑动平均模型'''
'''提供衰减率decay'''
...
# 定义一个滑动平均的类,初始化时 给定衰减率(0.99)和控制衰减率的变量step.
...
with tf.Session() as sess:
    ...
    # 根据公式shadow_variable = decay * shadow_variable + (1 - decay) * variable
    ...
    # 根据公式shadow_variable = decay * shadow_variable + (1 - decay) * variable
    # v1的滑动平均会被更新为 0.99*4.5 + 0.01*10 = 4.555
    sess.run(maintain_averages_op)
    print(sess.run([v1,ema.average(v1)]))

    # 在次更新滑动平均值,得到新滑动平均值为0.99*4.555 + 0.01*10 = 4.60945
    # 在次更新滑动平均值,得到新滑动平均值为0.99*4.60945 + 0.01*10 = 4.66335
